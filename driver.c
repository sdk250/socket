	...
	if (http) {
		...
		if (strstr(_buffer, "http://")) {
			...
			if (temp != NULL || strstr(temp->data, "Host: ")) {
				...
				temp->prev = root->next;
				root->next->data = (char *) malloc(sizeof(char) * host_len);
				memset(root->next->data, '\0', host_len);
				strcpy(root->next->data, host);
				root->next->prev = root;
				root->next->next = temp;
			}
			...
			if (temp != NULL || strstr(temp->data, "User-Agent: ")) {
				...
				*(temp->data + agent_len - 2) = '\0';
				...
				root->next = (struct http_header *) malloc(sizeof(struct http_header));
				temp->prev = root->next;
				char *tmp = "User-Agent: baiduboxapp\r\n";
				root->next->data = (char *) malloc(sizeof(char) * strlen(tmp));
				memset(root->next->data, '\0', strlen(tmp));
				strcpy(root->next->data, tmp);
				root->next->prev = root;
				root->next->next = temp;
			}
		} else {
